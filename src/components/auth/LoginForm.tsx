  ...
  return (
    <Card className="w-full shadow-xl">
      <CardHeader className="text-center">
        <CardTitle className="text-3xl font-bold text-primary">{t('loginTitle')}</CardTitle>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Button type="submit" className="w-full" disabled={authLoading || isGoogleLoading}>
        ...
        <Button variant="outline" className="w-full mt-6" onClick={handleGoogleLogin} disabled={authLoading || isGoogleLoading}>
          {isGoogleLoading ? (
            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
          ) : (
      ...
      <CardFooter className="flex justify-center">
        <p className="text-sm text-muted-foreground">
          {t('dontHaveAccount')}{' '}
          <Link href="#" className="font-semibold text-primary hover:underline" onClick={() => alert('Sign up not implemented yet.')}>
            {t('signUp')}
          </Link>
        </p>
      </CardFooter>
